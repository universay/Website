...
$OP.init = function () {
	...
	//unhandledrejection default handler. This is overridden if stacktrace is loaded on $OP.init().
	...
	window.addEventListener('load', function () {
		self.addScript($OP.baseURL("assets/js/vendor/mousetrap-master/mousetrap.min.js"), function () {
			self.addScript($OP.baseURL("assets/js/op_helpers.js"), function () {
				self.setupKeys();
			})
		})
	});
...
//quick script added
$OP.addScript = function (url, onloadF = function () { }) {
...
$OP.setupKeys = function () {
	...
	Mousetrap.bind(OPHELPERS.keyboardShortcuts.fullscreen.bind, function (e) {
		$OP.callParentFunction('mousetrap', OPHELPERS.keyboardShortcuts.fullscreen.bind);
		return false;
	});
	//save
	Mousetrap.bind(OPHELPERS.keyboardShortcuts.save.bind, function (e) {
		$OP.callParentFunction('mousetrap', OPHELPERS.keyboardShortcuts.save.bind);
		return false;
	});
	//exit fullscreen
	...
	Mousetrap.bind(OPHELPERS.keyboardShortcuts.play.bind, function (e) {
		$OP.callParentFunction('mousetrap', OPHELPERS.keyboardShortcuts.play.bind);
		return false;
	});
	//code
	Mousetrap.bind(OPHELPERS.keyboardShortcuts.code.bind, function (e) {
		$OP.callParentFunction('mousetrap', OPHELPERS.keyboardShortcuts.code.bind);
		return false;
	});
	//settings
	Mousetrap.bind(OPHELPERS.keyboardShortcuts.settings.bind, function (e) {
		$OP.callParentFunction('mousetrap', OPHELPERS.keyboardShortcuts.settings.bind);
		return false;
	});
	//layout
	Mousetrap.bind([OPHELPERS.keyboardShortcuts.layout.bind], function (e) {
		$OP.callParentFunction('mousetrap', OPHELPERS.keyboardShortcuts.layout.bind);
		return false;
	});
...
$OP.uploadRecording = function (blob, extension) {
	...
	// $OP.PREVIEW_BASE_URL doesn't exist on HTMl sketches, so set it to prod. When testing, use only p5js sketches
	let url = window.$OP.PREVIEW_BASE_URL ? window.$OP.PREVIEW_BASE_URL + '/sketch/uploadRecording_ajax' : 'https://preview-local.openprocessing.org/sketch/uploadRecording_ajax';
	...
	fetch(url, { // Your POST endpoint
		...
		json => {
			...
			$OP.callParentFunction('recordingReady', {
				'filename': filename,
				'extension': extension
			});
		}
